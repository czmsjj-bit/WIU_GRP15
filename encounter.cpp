...
void Encounter::GenerateEncounter()        // New Shit
{
    ...
    std::array<ScrollTemplate, 8> pool = { {
...
void Encounter::encounterDisplay()
{
    ...
    std::cout << "Souls remaining: " << soul::GetSoulCount() << "\n";
}
...
void Encounter::scrollPick()
{
    ...
    // Accessing an encounter costs one soul. If the player doesn't have one,
    // refuse entirely -- don't even show them the choices.
    if (!soul::GetSoulCount())
    {
        std::cout << "\nNo souls to visit Encounter (You have 0 souls)\n";
        system("pause");
        return;
    }
